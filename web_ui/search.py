...
def render(viewmodel: dict):
    ...
    async def process_prompt():
        if searchmodel[F_PROMPT]:  
            ...
            if task == "settings":
                ...
                banner = f"{len(items)} items found: {searchmodel[F_PROMPT]}" if items else messages.NOTHING_FOUND
    ...
    BindablePaginatedList(render_item).bind_contents_from(searchmodel, F_SEARCH_RESULT)
