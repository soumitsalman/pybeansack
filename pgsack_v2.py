...
import pandas as pd
from onlymaps import connect, Database, Bulk
...
_TABLES = {
    ...
    CHATTERS: Chatter,
    "_materialized_chatter_aggregates": Chatter,
...
def _to_dict(items: list) -> tuple[list[str], list[dict]]:
...
class Beansack:
    ...
    def store_beans(self, beans: list[Bean]):
        ...
        columns, to_store = _to_dict(distinct(to_store, K_URL))
        ...
        SQL_INSERT = f"""
        INSERT INTO beans ({fields}) VALUES ({placeholders})
        ON CONFLICT (url) DO NOTHING;
        """
        return self.db.exec(SQL_INSERT, Bulk(to_store))
    ...
    def update_beans(self, beans: list[Bean], columns: list[str] = None):
        ...
        return self.execute(update(_Bean), updates)
    ...
    def update_publishers(self, publishers: list[Publisher]):
        ...
        return self.execute(update(_Publisher), updates)
    ...
    def count_rows(self, table: str) -> int:
        ...
        return self.db.fetch_one(int, SQL_COUNT)
    
    # MAINTENANCE METHODS
    def refresh_classifications(self):        
        return self.execute(UPDATE_CLASSIFICATIONS)
        
    def refresh(self):
        with self.db.transaction():
            self.db.exec(UPDATE_CLASSIFICATIONS)
            self.db.exec(REFRESH_VIEWS)

    def execute(self, sql, *args):
        return self.db.exec(sql, *args)
    
    def store_parquet(self, file_path: Path, table_name: str, override: bool = True):
        ...
        with self.db.transaction():
            if override: self.db.exec(f"TRUNCATE TABLE {table_name};")
            self.db.exec(
                f"INSERT INTO {table_name} ({fields}) VALUES ({placeholders});",
                Bulk(df.to_dict('records'))
            )
...
def create_db(conn_str: str, factory_dir: str) -> Beansack:
    ...
    db.store_parquet(factory_path / "categories.parquet", "fixed_categories", True)
    db.store_parquet(factory_path / "sentiments.parquet", "fixed_sentiments", True)
    return db
