...
def get_topics(userid: str|tuple[str, str]):
    return sorted(categories.distinct("text", {K_SOURCE:userid}))
    
def update_topics(userid: str|tuple[str, str], topics: list[str]|dict[str, str]):
    userid = get_userid(userid)
    categories.delete_many({K_SOURCE: userid})
    if isinstance(topics, dict):
        categories.insert_many([{K_TEXT: topic, K_DESCRIPTION: description, K_SOURCE: userid } for topic, description in topics.items()], ordered=False)
