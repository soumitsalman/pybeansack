...
@cached(TTLCache(maxsize=CACHE_SIZE, ttl=ONE_HOUR))
def trending_tags(categories: str|tuple[str], kinds: str|tuple[str], last_ndays: int, topn: int) -> list[Bean]:
    return beansack.query_top_tags(filter=_create_filter(categories, None, kinds, last_ndays), limit=topn)
    
@cached(TTLCache(maxsize=CACHE_SIZE, ttl=ONE_HOUR))
def search(query: str, categories: str|tuple[str], tags: str|tuple[str], kinds: str|tuple[str], last_ndays: int, start_index: int, topn: int):
