...
def get_beans(urls: str|list[str], tags: str|list[str]|list[list[str]], kinds: str|list[str], sources: str|list[str], last_ndays: int, start: int, limit: int) -> list[Bean]:
    ...
    return beansack.get_beans(filter=filter, skip=start, limit=limit)
...
def vector_search_beans(query: str, accuracy: float, tags: str|list[str]|list[list[str]], kinds: str|list[str], sources: str|list[str], last_ndays: int, start: int, limit: int):
    ...
    if is_valid_url(query):
        ...
        return beansack.vector_search_beans(embedding=bean[K_EMBEDDING], min_score=accuracy, filter=filter, skip=start, limit=limit, projection=PROJECTION) if bean else []
    if query:
        return beansack.vector_search_beans(query=query, min_score=accuracy, filter=filter, skip=start, limit=limit, projection=PROJECTION) 
    return []
