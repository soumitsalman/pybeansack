...
def trending(urls: list[str], categories: str|list[str], tags: str|list[str], kinds: str|list[str], last_ndays: int, start_index: int, topn: int):
    ...
    filter=_create_filter(urls, categories, tags, kinds, last_ndays, None, None)
...
def trending_tags(urls: list[str], categories: str|list[str], kinds: str|list[str], last_ndays: int, start_index: int, topn: int) -> list[Bean]:
    return beansack.query_trending_tags(filter=_create_filter(urls, categories, None, kinds, last_ndays, None, None), skip=start_index, limit=topn)
...
def count_beans(query: str, urls: list[str], categories: str|list[str], tags: str|list[str], kind: str|list[str], last_ndays: int, topn: int) -> int:
    filter = _create_filter(urls, categories, tags, kind, last_ndays, None, None)
    if query:
...
def related(cluster_id: str, url: str, topn: int):
...
@cached(max_size=CACHE_SIZE, ttl=FOUR_HOURS)
def count_related(cluster_id: str, url: str, topn: int) -> int:
    filter = _create_filter(None, None, None, None, None, cluster_id, url)
    return beansack.beanstore.count_documents(filter=filter, limit=topn)
    
def _create_filter(urls: list[str], categories: str|list[str], tags: str|list[str], kinds: str|list[str], last_ndays: int, cluster_id: str, ignore_url: str) -> dict:
    filter = {
