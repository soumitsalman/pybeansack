...
def search(query: str, tags: str|tuple[str], kinds: str|tuple[str], last_ndays: int, min_score: float, start_index: int, topn: int):
    ...
    if query:
        return beansack.vector_search_beans(embedding=llmops.embed(query), min_score=min_score, filter=filter, sort_by=LATEST_AND_TRENDING, limit=topn, projection=PROJECTION)    
